...
internal static class Program
{
    ...
    {
        const string script = "$gpu = Get-CimInstance Win32_PnPEntity | Where-Object { $_.PNPDeviceID -like 'PCI\\VEN_1002*' -and ($_.PNPClass -eq 'Display' -or $_.ClassGuid -eq '{4d36e968-e325-11ce-bfc1-08002be10318}' -or $_.Service -match 'BasicDisplay|amdwddmg|amdkmdag') } | Sort-Object @{ Expression = { if ($_.PNPDeviceID -match 'DEV_(1114|13C0|15BF|15C8|164C|164D|164E|1900|1901|1902)') { 1 } else { 0 } } }, Name | Select-Object -First 1; $cpu = Get-CimInstance Win32_Processor | Select-Object -First 1; [Console]::WriteLine([Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes([string]$gpu.Name))); [Console]::WriteLine([Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes([string]$gpu.PNPDeviceID))); [Console]::WriteLine([Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes([string]$cpu.Name)))";
